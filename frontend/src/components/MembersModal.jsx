...
export default function MembersModal({ drpg, region, compartmentId, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm sm:p-4">
      <div className="bg-white sm:rounded-2xl shadow-2xl w-full max-w-[77rem] h-[92vh] sm:max-h-[85vh] flex flex-col rounded-t-2xl">
        {/* Header */}
            ...
            <>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {members.map((m, idx) => {
                      ...
                      return (
                        <tr key={m.member_id || idx} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                          ...
                          <td className="py-3 px-4">
                            <span className="text-gray-800 font-medium text-sm">
                              {m.display_name && m.display_name !== m.member_id ? m.display_name : m.member_id || '—'}
                            </span>
                            {m.display_name && m.display_name !== m.member_id && (
                              <p className="text-xs text-gray-400 font-mono truncate max-w-sm">{m.member_id}</p>
                            )}
                          </td>
                          <td className="py-3 px-4">
                            {m.lifecycle_state ? <StateBadge state={m.lifecycle_state} /> : <span className="text-gray-400 text-xs">—</span>}
                          </td>
                          <td className="py-3 px-4"><RpoBadge value={m.rpo} /></td>
                          <td className="py-3 px-4 text-xs text-gray-500 whitespace-nowrap">
                            {m.last_sync ? new Date(m.last_sync).toLocaleString()
                              : m.apply_lag_raw ? <span className="text-orange-600 font-mono">{m.apply_lag_raw}</span>
                              : '—'}
                          </td>
                        </tr>
                      )
              ...
              <div className="sm:hidden space-y-3">
                {members.map((m, idx) => {
                  const Icon = MEMBER_TYPE_ICONS[m.member_type] || Box
                  return (
                    <div key={m.member_id || idx} className="border border-gray-100 rounded-xl p-4 space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {m.lifecycle_state && <StateBadge state={m.lifecycle_state} />}
                      </div>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-gray-400">
                          {m.last_sync ? new Date(m.last_sync).toLocaleDateString()
                            : m.apply_lag_raw ? <span className="text-orange-600 font-mono">{m.apply_lag_raw}</span>
                            : '—'}
                        </span>
                      </div>
        ...
        <div className="p-4 border-t border-gray-100 flex justify-end safe-bottom">
